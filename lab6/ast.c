...
//-------------------------------[CALL NODE CASE]--------------------------------------------//
        case A_CALL:
                    ...
                    PT(level + 1); printf("(\n");
                    if (p->s1 != NULL) {
                        /* p->s1 may be an A_ARG_LIST node or an EXPR */
                        if (p->s1->nodetype == A_ARG_LIST) {
                            ASTprint(level, p->s1);
                        } else {
                            PT(level + 2); printf("CALL ARGUMENT(S)\n");
                ...
                break;
//-------------------------------[ARG LIST NODE CASE]--------------------------------------------//
        case A_ARG_LIST:  //internal component - prints each argument with a header
                    PT(level);
                    ASTprint(level + 1, p->s1);
                    if (p->s2 != NULL) {
                        /* more arguments follow at same level */
                        ASTprint(level, p->s2);
                    }
                break;
...
//-------------------------------[FUNCTION DECLARATION NODE CASE]---------------------------------//            
        ...
        case A_FUNDEC:  
                    PT(level);
                    /* Print as: Function <type>  <name>(VOID) */
...
//------------------------------[PARAM NODE CASE]--------------------------------------------//
        case A_PARAM:
                    if (p->value == 1) {
                        PT(level);
                        printf("PARAMETER: %s[%d] \n", p->name, p->value); //array parameter
                    } else {
                        PT(level);
                        printf("PARAMETER: %s %s \n", DataTypeToString(p->datatype), p->name); //normal parameter
                    }
                break;
//------------------------------[WHILE NODE CASE]-----------------------------------------------//
        case A_ITERATION:
